...
# 설정
# =========================
ROOT_DIR = "../GOOGLE_API_DATA/people_docs_crawling/people_docs_crawled"
OUT_JSONL = "./people_qa_dataset_set5.jsonl"  # 결과를 저장할 JSONL 파일
MODEL = "gpt-4o-mini"
...
# tiktoken 라이브러리의 get_encoding() 함수를 사용해
# cl100k_base라는 이름의 인코딩 객체를 enc 변수에 할당
# cl100k_base: GPT-4, GPT-3.5-Turbo, GPT-4o-mini 같은 최신 OpenAI 모델들이 사용하는 토큰화 인코딩 방식
...
# 모델 호출
# =========================
def ask_model(pair_text, n, source_url, prev_qs=None):
    """
...
def build_record(q, a, doc_path, pair_index,
    ...
    # 사용 안하니까 삭제하면 어떨지?
    # rid = hash_id(doc_path, str(pair_index), q, a)
    # preview = (pair_text or "").strip().replace("\n", " ")[:240]
    return {
        "question": q.strip(),
        "answer": a.strip(),
        "sources": [source_url or f"file://{doc_path}"],
        "tags": "people",
...
def process_one_file(file_path, out_fh):
    ...
    if len(chunks) == 1:
        ...
        items = ask_model(pair_text, PAIR_MAX_QA, source_url, prev_qs=asked_qs_for_model)
        for it in items:
            ...
            rec = build_record(q, a, file_path, 0, [0], pair_text, source_url)
    ...
    for (pair_idx, cg) in pairs:
        ...
        for it in items:
            ...
            rec = build_record(q, a, file_path, pair_idx, chunk_indices, pair_text, source_url)
            out_fh.write(json.dumps(rec, ensure_ascii=False) + "\n")
            written += 1
