...
이 시스템은 아래와 같이 크게 **구글 API 전문 메인 챗봇**과 **사내 내부 문서 전문 sLLM 챗봇**으로 구성됩니다.  
...
- **API 키 관리**:  
  > API 키의 **생성, 갱신, 삭제** 등을 **중앙화된 관리 시스템**을 통해 처리하는 기능입니다. 사용자는 자신의 API 키를 관리하고, 사용 이력을 확인할 수 있습니다. 또한, **보안 기능**이 강화되어 키 노출을 방지하며, 필요에 따라 **권한 설정**을 통해 특정 API에 대한 접근을 제한할 수 있습니다. 이를 통해 보안 관리 및 운영 효율성을 극대화할 수 있습니다.  
...
<**모델**>: **QWEN 8B** 모델을 기반으로 사내 문서에 대한 질의응답 시스템을 구축합니다.  
  - **파인튜닝**: 사내 문서에 특화된 **멀티턴 데이터셋**을 생성하여 모델을 최적화합니다.  
  - **말투 학습**: 사내의 고유한 문서 말투를 모델이 반영하도록 파인튜닝합니다.  
